...
/*
 * @class Control.LayerControl
 * @aka L.Control.LayerControl
 * @inherits Control
 *
 * The layers control gives users the ability to switch between different base layers and switch overlays on/off 
 * (check out the [detailed example](http://leafletjs.com/examples/layers-control/)). Extends `Control`.
 *
 * @example
 *
 * ```js
 ...
 * };
 *
 * L.control.layerControl(baseLayers, overlays).addTo(map);
 * ```
 */
export const LayerControl = Control.extend({
	// @section
	...
	initialize: function (baseLayers, overlays, options) {
		...
		for (let i in baseLayers) {
			for (let m in baseLayers[i]) {
				this._addLayer(baseLayers[i][m], m, i);
			}
		}

		for (let o in overlays) {
			for (let n in overlays[o]) {
	...
	_addItem: function (obj) {
		...
		name.className = 'leaflet-control-layers-name'
...
export const layerControl = function (baseLayers, overlays, options) {
	return new LayerControl(baseLayers, overlays, options);
};
