...
public class WSGeometry {
	...
	private Set<Geometry> getGeometryImpl(String sessionId, Long objTypeId, String bbox) throws SvException {

		Set<Geometry> geom = null;

		try (SvGeometry svg = new SvGeometry(sessionId)) {
			// Why init SvGeometry on each call? can we make-do with a single core instance?
			geom = (Set<Geometry>) svg.getGeometriesByBBOX(objTypeId, bbox);
		}
		return geom;
	}
	...
	public StreamingOutput getGeometry(@PathParam("token") final String token, @PathParam("bbox") final String bbox,
			@PathParam("objectName") final String objectName) {

		// token and bbox are a must, that is the point of the service. To consider:
		// Should we include objectName to signature? why don't we just get all vectors
		// for the given bounding box?

		// Objection 1: Because all tables render on different scales of the map.
		// Parcels render < 1:5000.
		// If we omit type, we must filter each row for the current scale (which becomes
		// an argument). This may be way worse.

		// Objection 2: Most tables will not render vectors at all for a given session,
		// consequence of user type.
		// Farmers should not get full built SVG clickable reference parcels. They will
		// get a background image via the image server.
		return new StreamingOutput() {
			public void write(OutputStream stream) {
				...
				try {
					...
					String errMsg = "Failed fetching geometry set with type: " + objectName
	...
	public Response getOrigin(@PathParam("token") String token) {
		...
		try {
			...
			// Theoretically, the token carries information about the user, we start with
			// that.

			// We get all geometries pertaining to the user (whatever pertaining means to
			// the system).
			// We generate an envelope of the full geometry set.
			// Then we create a (string) bounding box and assign to origin => return.

			// What this service does is a calculation (executed once) of a specific
			// geographical space,
			// represented by a bbox (i.e. frame), that is relevant to the user.

			// If a beneficiary makes access in the application, we get all registered
			// parcels which are
			// owned by him, calculate the smallest geographical rectangle that contains the
			// parcels
